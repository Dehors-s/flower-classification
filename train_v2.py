...
class WideResNet(nn.Module):
    ...
    def __init__(self, depth=28, widen_factor=4, dropout_rate=0.3, num_classes=102):
...
def create_data_loaders(data_dir, batch_size=32):
    ...
    # 创建数据集
    image_datasets = {
        'train': datasets.ImageFolder(os.path.join(data_dir, 'train_augmented'),
                                      data_transforms),
    ...
    dataloaders = {
        ...
        'test': torch.utils.data.DataLoader(
            image_datasets['test'], batch_size=batch_size, shuffle=False,
            num_workers=4, pin_memory=True
        )
    }
...
def train_model(model, dataloaders, dataset_sizes, criterion, optimizer, scheduler, num_epochs=100,
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            pbar = tqdm(total=batch_count, desc=f'{phase}批次', unit='batch')
