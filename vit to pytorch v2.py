...
# 从vit_v2导入配置
try:
    from vit_v2 import Config, ImprovedViT

    print("✅ 成功导入vit_v2配置和模型")
except ImportError as e:
    print(f"❌ 导入vit_v2失败: {e}")
    print("请确保vit_v2.py在相同目录下")
    sys.exit(1)


# PyTorch版本的ImprovedViT模型定义（与PaddlePaddle版本结构相同）
class PyTorchImprovedViT(nn.Module):
...
class PyTorchStochasticDepth(nn.Module):
    """PyTorch版本的随机深度"""
...
class PyTorchTransformerBlock(nn.Module):
    def __init__(self, dim, heads, mlp_dim, dropout=0.1, stochastic_depth_rate=0.0):
        ...
        self.stochastic_depth = PyTorchStochasticDepth(stochastic_depth_rate)
        self.dropout = nn.Dropout(dropout)
...
def create_paddle_model_from_vit_v2():
    """从vit_v2创建PaddlePaddle模型"""
    model = ImprovedViT(
...
def convert_paddle_to_pytorch(paddle_model_path=None):
    ...
    if not os.path.exists(paddle_model_path):
        ...
        possible_paths = [
            ...
            os.path.join(os.path.dirname(__file__), 'output_vit_improved', 'best_model.pdparams'),
            os.path.join(os.path.dirname(__file__), 'checkpoint_epoch_15.pdparams'),
            'best_model.pdparams',
    ...
    paddle_model = create_paddle_model_from_vit_v2()
    ...
    try:
        ...
        # 检查检查点类型
        if 'model_state_dict' in paddle_state_dict:
            paddle_state_dict = paddle_state_dict['model_state_dict']
    ...
    # 创建PyTorch模型
    pytorch_model = PyTorchImprovedViT(
        image_size=Config.image_size,
        patch_size=16,
        num_classes=Config.num_classes,
        dim=384,
