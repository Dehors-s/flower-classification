...
def evaluate_model(model, dataloader, class_names):
    ...
    # 添加进度条
    pbar = tqdm(total=len(dataloader), desc='验证批次', unit='batch')
...
if __name__ == "__main__":
    ...
    model = WideResNet(
        depth=28,  # 减小网络深度
        widen_factor=4,  # 减小宽度因子
        dropout_rate=0.4,  # Dropout率
        num_classes=len(class_names)  # 根据实际类别数调整
    )
    ...
    model, train_loss, train_acc, val_loss, val_acc = train_model(
        ...
        num_epochs=100, use_amp=False  # 禁用混合精度
    )
    ...
    # 在验证集上评估模型（替代原来的测试集评估）
    print("\n在验证集上评估模型...")
    val_accuracy, preds, labels, probs = evaluate_model(model, dataloaders['val'], class_names)
